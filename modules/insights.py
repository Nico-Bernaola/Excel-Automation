...
def _request_gemini(context: str, api_key: str) -> str:
        ...
        "contents": [{
            "parts": [{
                "text": (
                    ...
                    "Analyze these sales data and write a brief executive summary: "
                    "which salesperson performed best, which product sold the most, "
                    "if there's anything noteworthy or requiring attention. "
    ...
    try:
        ...
        raise RuntimeError(f"Gemini Error({e.code}): {e.read().decode()}")
